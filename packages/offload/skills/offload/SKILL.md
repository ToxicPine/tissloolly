...
- First, look at the user's environment for `FOOLFAD_TRANSPORT` (e.g. `foolfad-ssh box.lab`,
  `foolfad-tailscale box.lab`, or `foolfad-fly --app … --machine …`). If it's set, a computer is
  already set up and pointed to — use it.
- If it isn't set, say so plainly — something like: *"There's no machine listed in your
