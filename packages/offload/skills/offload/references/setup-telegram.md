...
```bash
nix run github:ToxicPine/ambit -- secrets set <machine>.<network> TELEGRAM_BOT_TOKEN=<token> --json
nix run github:ToxicPine/ambit -- secrets set <machine>.<network> TELEGRAM_ALLOWED_USERS=<id> --json
```
...
```bash
nix run github:ToxicPine/ambit -- secrets set <machine>.<network> TELEGRAM_HOME_CHANNEL=<id> --json
```
...
`nix run github:ToxicPine/ambit -- secrets set ... --json`, out of every project. Machine
